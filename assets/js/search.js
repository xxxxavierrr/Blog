...
// Fix: Use 'input' event instead of 'keyup' and handle IME composition
// This ensures proper Chinese/IME input handling
    ...
    if (httpRequest.readyState === 4) {
      if (httpRequest.status === 200) {
        try {
          ...
          console.error("Search: Failed to parse index.json", e);
        }
      } else {
        console.error("Search: Failed to fetch index.json, status:", httpRequest.status);
      }
    }
  };
...
function buildIndex() {
  ...
  fetchJSON(baseURL + "index.json", function (data) {
    var options = {
      shouldSort: true,
      ignoreLocation: true,
      // Fix 1: Changed from 0.0 to 0.4 — enables actual fuzzy search
      // 0.0 = exact match only (unusable), 0.6 = default, 0.4 = balanced for CJK
      threshold: 0.4,
      includeMatches: true,
      // Fix 2: Enable extended search for better matching
      useExtendedSearch: true,
      // Fix 3: Ignore field length normalization (helps short Chinese titles)
...
function executeQuery(term) {
  // Fix 4: Guard against fuse not being initialized yet
  if (!fuse) {
    output.innerHTML = "";
    return;
  }

  // Fix 5: Handle empty/whitespace query
